...
static Tr_exp Tr_Ex(T_exp ex)		// 合成Ex类型的树的节点
{
...
static Tr_exp Tr_Nx(T_stm nx)		// 合成Nx类型的树的节点
{
...
static Tr_exp Tr_Cx(patchList trues, patchList falses, T_stm stm)	// 合成Cx类型的树的节点
{
...
static T_exp unEx(Tr_exp e)		// 拆开树的节点使其为Ex
{
	switch(e->kind) {
		case Tr_ex:
			return e->u.ex;
...
static T_stm unNx(Tr_exp e)		// 拆开树的节点使其为Nx
{
	switch(e->kind) {
		case Tr_ex:
			return T_Exp(e->u.ex);
...
static struct Cx unCx(Tr_exp e)		// 拆开树的节点使其为Cx
{
	switch(e->kind) {
		case Tr_ex:
		{
...
void Tr_ExpList_append(Tr_expList list, Tr_exp expr)	// 在list的尾部添加expr
{
	if (list->head) {
...
void Tr_ExpList_prepend(Tr_expList list, Tr_exp expr)	// 在list的头部添加expr
{
	if (list->head) {
		Tr_node node = checked_malloc(sizeof(*node));
		node->expr = expr;
...
Tr_exp Tr_seqExp(Tr_expList list)			// 将从后至前的list再反回来变成从前至后, 并构成ESEQ
{
...
Tr_exp Tr_simpleVar(Tr_access access, Tr_level level)
{
	T_exp addr = T_Temp(F_FP());
	/* Follow static links until we reach level of defintion */
	while (level != access->level) {							// 如果不在当前level
		/* Static link is the first frame formal */
		F_access staticLink = F_formals(level->frame)->head;	// 到静态链中查找(静态链是栈的第一个)
		addr = F_Exp(staticLink, addr);
		level = level->parent;
	}
	return Tr_Ex(F_Exp(access->access, addr));					// 返回TEMP或者MEM
}
...
void Tr_procEntryExit(Tr_level level, Tr_exp body, bool returnValue)
{   
	T_stm stm = NULL;
	FILE* out = fopen("frag.txt", "w");
	FILE* json_out = fopen("display/js/json_frag.js", "w");
	if (returnValue) 
