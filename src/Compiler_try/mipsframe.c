...
static F_accessList makeFormalAccessList(F_frame f, U_boolList formals)
{
	...
	for (p = formals; p; p = p->tail, argRegs = argRegs->tail) {
		...
		if (argRegs != NULL) {
			f->argList = T_StmList(T_Move(F_Exp(access, T_Temp(F_FP())),
				T_Temp(argRegs->head)), f->argList);
		}
	}
	return headList;
}
...
{
	//计算新sp->保存原fp->计算新fp->保存 ra->保存Callee-save寄存器->保存参数->(函数体原指令)->
	//恢复Callee-save寄存器->恢复返回地址->恢复fp->恢复sp
