...
{
	for (; stmList; stmList = stmList->tail) {
		json_stm(out, stmList->head, 0); fprintf(out, "\n");
	}
}
...
void json_stm(FILE *out, T_stm stm, int d)
{
	...
	case T_SEQ:
		...
		indent(out, d + 1); fprintf(out, "name: \"SEQ\",\n");
		indent(out, d + 1);	fprintf(out, "children: [\n");
		json_stm(out, stm->u.SEQ.left, d + 2);  fprintf(out, ",\n");
		json_stm(out, stm->u.SEQ.right, d + 2);	fprintf(out, "\n");
		indent(out, d + 1); fprintf(out, "]\n");
		indent(out, d);		fprintf(out, "}");
		break;
	case T_LABEL:
		indent(out, d);		fprintf(out, "{\n");
		indent(out, d + 1); fprintf(out, "name: \"LABEL\",\n");
		indent(out, d + 1); fprintf(out, "children: [\n");
		indent(out, d + 2);	fprintf(out, "{\n");
		indent(out, d + 3); fprintf(out, "name: \"%s\"\n", S_name(stm->u.LABEL));
		indent(out, d + 2);	fprintf(out, "}\n");
		indent(out, d + 1); fprintf(out, "]\n");
		indent(out, d);		fprintf(out, "}");
		break;
	case T_JUMP:
		indent(out, d);		fprintf(out, "{\n");
		indent(out, d + 1);	fprintf(out, "name: \"JUMP\",\n");
		indent(out, d + 1); fprintf(out, "children: [\n");
		json_tree_exp(out, stm->u.JUMP.exp, d + 2);	fprintf(out, "\n");
		indent(out, d + 1); fprintf(out, "]\n");
		indent(out, d);		fprintf(out, "}");
		break;
	case T_CJUMP:
		indent(out, d);		fprintf(out, "{\n");
		indent(out, d + 1);	fprintf(out, "name: \"CJUMP\",\n");
		indent(out, d + 1); fprintf(out, "children: [\n");
		indent(out, d + 2); fprintf(out, "{\n");
		indent(out, d + 3); fprintf(out, "name: \"%s\"\n", rel_oper[stm->u.CJUMP.op]);
		indent(out, d + 2); fprintf(out, "},\n");
		json_tree_exp(out, stm->u.CJUMP.left, d + 2);	fprintf(out, ",\n");
		json_tree_exp(out, stm->u.CJUMP.right, d + 2);	fprintf(out, ",\n");
		indent(out, d + 2); fprintf(out, "{\n");
	...
	case T_MOVE:
		indent(out, d);		fprintf(out, "{\n");
		indent(out, d + 1);	fprintf(out, "name: \"MOVE\",\n");
		indent(out, d + 1);	fprintf(out, "children: [\n");
		json_tree_exp(out, stm->u.MOVE.dst, d + 2);	fprintf(out, ",\n");
		json_tree_exp(out, stm->u.MOVE.src, d + 2);	fprintf(out, "\n");
		indent(out, d + 1); fprintf(out, "]\n");
		indent(out, d);		fprintf(out, "}");
		break;
	case T_EXP:
		indent(out, d);		fprintf(out, "{\n");
		indent(out, d + 1);	fprintf(out, "name: \"EXP\",\n");
		indent(out, d + 1);	fprintf(out, "children: [\n");
		json_tree_exp(out, stm->u.EXP, d + 2);	fprintf(out, "\n");
		indent(out, d + 1); fprintf(out, "]\n");
		indent(out, d);		fprintf(out, "}");
		break;
	}
...
void json_tree_exp(FILE *out, T_exp exp, int d)
{
	...
	case T_BINOP:
		indent(out, d);		fprintf(out, "{\n");
		indent(out, d + 1);	fprintf(out, "name: \"BINOP\",\n");
		indent(out, d + 1);	fprintf(out, "children: [\n");
		indent(out, d + 2); fprintf(out, "{\n");
		indent(out, d + 3); fprintf(out, "name: \"%s\"\n", bin_oper[exp->u.BINOP.op]);
		indent(out, d + 2); fprintf(out, "},\n");
		json_tree_exp(out, exp->u.BINOP.left, d + 2);	fprintf(out, ",\n");
		json_tree_exp(out, exp->u.BINOP.right, d + 2);	fprintf(out, "\n");
		indent(out, d + 1); fprintf(out, "]\n");
		indent(out, d);		fprintf(out, "}");
		break;
	case T_MEM:
		indent(out, d);		fprintf(out, "{\n");
		indent(out, d + 1);	fprintf(out, "name: \"MEM\",\n");
		indent(out, d + 1);	fprintf(out, "children: [\n");
		json_tree_exp(out, exp->u.MEM, d + 2);	fprintf(out, "\n");
		indent(out, d + 1); fprintf(out, "]\n");
		indent(out, d);		fprintf(out, "}");
		break;
	case T_TEMP:
		indent(out, d);		fprintf(out, "{\n");
		indent(out, d + 1);	fprintf(out, "name: \"TEMP\",\n");
		indent(out, d + 1); fprintf(out, "children: [\n");
		indent(out, d + 2);	fprintf(out, "{\n");
		indent(out, d + 3); fprintf(out, "name: \"%s\"\n", Temp_look(Temp_name(), exp->u.TEMP));
		indent(out, d + 2);	fprintf(out, "}\n");
	...
	case T_ESEQ:
		indent(out, d);		fprintf(out, "{\n");
		indent(out, d + 1);	fprintf(out, "name: \"ESEQ\",\n");
		indent(out, d + 1);	fprintf(out, "children: [\n");
		json_stm(out, exp->u.ESEQ.stm, d + 2);		fprintf(out, ",\n");
		json_tree_exp(out, exp->u.ESEQ.exp, d + 2);	fprintf(out, "\n");
		indent(out, d + 1); fprintf(out, "]\n");
		indent(out, d);		fprintf(out, "}");
		break;
	case T_NAME:
		indent(out, d);		fprintf(out, "{\n");
		indent(out, d + 1);	fprintf(out, "name: \"NAME\",\n");
		indent(out, d + 1); fprintf(out, "children: [\n");
		indent(out, d + 2);	fprintf(out, "{\n");
		indent(out, d + 3); fprintf(out, "name: \"%s\"\n", S_name(exp->u.NAME));
		indent(out, d + 2);	fprintf(out, "}\n");
	...
	case T_CONST:
		indent(out, d);		fprintf(out, "{\n");
		indent(out, d + 1);	fprintf(out, "name: \"CONST\",\n");
		indent(out, d + 1); fprintf(out, "children: [\n");
		indent(out, d + 2);	fprintf(out, "{\n");
		indent(out, d + 3); fprintf(out, "name: \"%d\"\n", exp->u.CONST);
		indent(out, d + 2);	fprintf(out, "}\n");
		indent(out, d + 1); fprintf(out, "]\n");
		indent(out, d);		fprintf(out, "}");
		break;
	case T_CALL:
		indent(out, d);		fprintf(out, "{\n");
		indent(out, d + 1);	fprintf(out, "name: \"CALL\",\n");
		indent(out, d + 1);	fprintf(out, "children: [\n");
		json_tree_exp(out, exp->u.CALL.fun, d + 2);
		for (T_expList args = exp->u.CALL.args; args; args = args->tail) {
			fprintf(out, ",\n"); 
			json_tree_exp(out, args->head, d + 2);
		}
		fprintf(out, "\n");
		indent(out, d + 1); fprintf(out, "]\n");
		indent(out, d);		fprintf(out, "}");
