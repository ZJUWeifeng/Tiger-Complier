...
#define BUFSIZE 8192
#define RESERVEDSIZE 19

char strbuf[BUFSIZE+1];
char *strptr = NULL;
unsigned int strlength = 0;

char replaceMsg[1024] = "";
...
[a-zA-Z][a-zA-Z0-9_]*    {adjust();
                          ...
                          if(parseErrors && (EM_tokPos == EM_errorPos || isInList(EM_tokPos))){
						      IntList lines = linePos; 
                              int num = lineNum;
						      yylval.sval = reserved[index].name;
							  if(EM_tokPos == EM_errorPos){
							      parseErrors = FALSE;
								  move_counter = 0;
							  }
                              ...
                              if (lines)
							      sprintf(replaceMsg, "%sFind syntax error at line:%d, pos:%d.\nThe compiler has replaced \"%s\" with \"%s\" to get rid of it.\n", replaceMsg, num, EM_tokPos-lines->i, yytext, yylval.sval);
